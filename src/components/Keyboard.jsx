        ...
        return <div key={i} className='flex justify-center w-full max-w-120 gap-1'>
          {
            keys.map((key, j) => {
              ...
              }} className={`flex border dark:border-[rgba(255,255,255,0.1)] lg:text-3xl justify-center items-center rounded lg:p-4  px-3 py-3 min-w-9 lg:min-w-12 ${keysStateRow[i][j] === 'R' ? 'bg-[#619d5c] dark:bg-[#7fb877] text-white' : keysStateRow[i][j] === 'F' ? 'bg-[#ddc459] dark:bg-[#c9a938] text-white' : keysStateRow[i][j] === 'N' ? 'text-white bg-gray-400 dark:bg-[#505a6b]' : 'bg-[#acdda8] dark:bg-[#2a3942] text-[#234120] dark:text-[#e0e8f0]'}  ${key === 'Enter' ? 'text-sm lg:text-xl' : 'text-xl lg:text-2xl'} cursor-pointer active:translate-y-0.5`} key={key}>{key === 'Backspace' ? <MdOutlineBackspace className='lg:size-9' /> : key}</div>
            })
          }
        </div>
      })}
