  ...
  return (
    <div className='flex z-50 gap-2 w-screen max-w-540 pt-2 md:pt-3 px-2 md-px-5'>
      <div className='shadow-[0_3px_0_0_#234120] flex gap-2 items-center justify-center px-[min(5rem,(calc(0.5rem+0.5vw)))] py-1 bg-[#acdda8] text-[calc(0.8rem+1vw)] min-w-25 min-h-10 font-bold text-[#234120] rounded-4xl cursor-pointer hover:bg-[#9ac596] duration-95 '>
        <p onClick={() => { setShowPopUp((prev) => (prev === 'Statistics' ? null : 'Statistics')) }} className={`pop-up-button ${locationPath === '/' ? 'animate-main-down' : 'animate-up hidden'} flex gap-1 justify-center items-center`}><GrPieChart className='-mt-1' /> Stastistics</p>
        <Link to='/' className={`pop-up-button ${locationPath === '/game-page' ? 'animate-main-up' : 'animate-down hidden'} flex gap-1 justify-center items-center`}><MdArrowBack /> Back</Link>
      </div>
